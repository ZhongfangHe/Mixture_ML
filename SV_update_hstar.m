...
% p(sig2) ~ G(0.5, 2*sig2_s), e.g. sig2_s = 10
%
% Use ASIS to improve estimate of mu, sig2, phi and h

function [h, mu, phi, sig] = SV_update_hstar(logy2, h, mu, phi,...
    sig, sig2_s, prior)
% Inputs:
...
phia = prior(3); phib = prior(4); % AR(1): p(phi) ~ 0.5*(1+betarnd(a,b))
sig2 = sig^2;
T = length(logy2);
...
Kh_covec = sig * invSigystar * (logy2 - d - mu); 
tmp = mvnrnd(Kh_covec,Kh)';
hstar = Kh\tmp;
